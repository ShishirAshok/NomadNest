...
export const register=async(req,res)=>{
    try{
        ...
        if(password.length<6) return resizeBy.status(400).json({success:false,message:'Password should be minimum 6 characters'})
        const emailLowerCase = email.toLowerCase();
        const existedUser=await User.findOne({email:emailLowerCase});
        if(existedUser) return res.status(400)
